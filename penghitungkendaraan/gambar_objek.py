...
WARNA_GREY = (255, 255, 255)
WARNA_GREY = (255, 255, 255)
WARNA_GREY = (133, 133, 133)
...
class GambarObjek():
    ...
    def gambar_box_n_centroid(self, frame, daftar_objek, greyscale=False):
        ####
        ...
        for (i, objek) in enumerate(daftar_objek):
            ...
            if not greyscale:
                ...
                # gambar centroid pada frame
                cv.circle(frame, centroid, 2, WARNA_GREY, -1)

        return frame

    def gambar_garis_tracking(self, frame, daftar_kendaraan, printteks=True, greyscale=False):
        ####
        ...
        for (i, kendaraan) in enumerate(daftar_kendaraan):
            ...
            if not greyscale:
                ...
                # gambar garis pergerakan
                for point in kendaraan.centroid:
                    cv.circle(frame, point, 2, WARNA_GREY, -1)
                    cv.polylines(frame, [np.int32(
                        kendaraan.centroid)], False, WARNA_GREY, 1)

                # gambar centroid pada frame
                cv.circle(frame, kendaraan.centroid[-1], 2, WARNA_GREY, -1)
        return frame

    def gambar_garis_batas(self, frame, greyscale=False):
        ####
    ...
    def tampilkan_frame(self, frame, foreground, frame_counter, daftar_objek, daftar_kendaraan, elapsed, jmlobjek, jmlkendaaan):
        ...
        frame_foreground_hasil = foreground.copy()

        frame_foreground_hasil = self.gambar_box_n_centroid(
            frame_foreground_hasil, daftar_objek, True)

        frame_foreground_hasil_garis = frame_foreground_hasil.copy()

        frame_foreground_hasil_garis = self.gambar_garis_tracking(
            frame_foreground_hasil_garis, daftar_kendaraan, False, True)
        ...
        self.tampilkan_simpan_jalan(
            'morfologi_hasil', frame_foreground_hasil, frame_counter)
        self.tampilkan_simpan_jalan(
            'morfologi_hasil_garis', frame_foreground_hasil_garis, frame_counter)
